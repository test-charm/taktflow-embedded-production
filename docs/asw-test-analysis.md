...
已建成 **29 条 ASW E2E 链**（`e2e-tests/src/test/resources/features/`，570 场景，原生 harness 链接真实 SWC 生产代码），覆盖以下 ASW 模型：
...
以下为**未**被现有 29 个 feature 覆盖、但具备补建条件的模块。
...
2. `RZC Swc_RzcNvm`（与 `cvc_nvm.feature` 对偶）
