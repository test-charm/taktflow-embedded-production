...
class BswComCfgPhase(BaseModel):
    """One phase of the BSW Com config readback harness script.

    Reads the arxmlgen-generated Com_Cfg_<Ecu>.c data tables back through the
    exported `const Com_ConfigType <ecu>_com_config` symbol. The tables are
    static const data generated from the DBC (GENERATED BY arxmlgen -- DO NOT
    EDIT), so no BSW logic is exercised — this verifies DBC fidelity of the
    generated configuration.
    """
    op: str = "dump"                 # dump|txpdu|rxpdu|sig|invariants
    id: int = 0                      # txpdu/rxpdu: PDU id; sig: signal id
    table: str = "tx"                # sig: tx|rx


class BswComCfgSetupBody(BaseModel):
    phases: list[BswComCfgPhase] = []


class BswComCfgRunBody(BaseModel):
    phases: list[BswComCfgPhase] | None = None  # stimulus phases, appended after stored precondition


class BswRteTaskBodiesPhase(BaseModel):
    """One phase of the BSW RTE task-bodies harness script.

    Drives one generated OSEK task body of Rte_TaskBodies_Cvc.c
    (GENERATED BY arxmlgen -- DO NOT EDIT, S-OS-11) with all external
    dependencies mocked: Os_TaskMap bridge, SWC/BSW runnables,
    WdgM_CheckpointReached, TerminateTask, Cvc_Os_StartScheduleTables. The
    call log verifies the exact S-OS-11 runnable dispatch order and WdgM
    checkpoint placement. The Idle task's infinite loop is bounded via
    setjmp/longjmp after `idleIters` Os_TaskMap calls.
    """
    op: str = "run"                  # run
    task: str = "1ms"                # 1ms|10ms|50ms|100ms|5000ms|idle
    idleIters: int = 3               # idle: bounded loop iterations


class BswRteTaskBodiesSetupBody(BaseModel):
    phases: list[BswRteTaskBodiesPhase] = []


class BswRteTaskBodiesRunBody(BaseModel):
    phases: list[BswRteTaskBodiesPhase] | None = None  # stimulus phases, appended after stored precondition
...
# Stored BSW Com config readback phase script for Given/When separation.
_stored_bsw_comcfg_cvc_phases: list[BswComCfgPhase] = []

# Stored BSW RTE task-bodies phase script for Given/When separation.
_stored_bsw_rtetaskbodies_cvc_phases: list[BswRteTaskBodiesPhase] = []
...
_BSW_COMCFG_CVC_HARNESS = "/app/bin/bsw_comcfg_cvc_harness"

_BSW_RTETASKBODIES_CVC_HARNESS = "/app/bin/bsw_rtetaskbodies_cvc_harness"
...
def _generate_coverage_html():
    ...
    harnesses = [
        ...
        # BSW RTE task bodies: the generated Rte_TaskBodies_Cvc.c HAS
        # executable functions and lives outside the ignored paths, so it
        # appears in the report with real line coverage.
        ("bsw_rtetaskbodies_cvc", _BSW_RTETASKBODIES_CVC_HARNESS),
        # NOTE: the BSW Com config harness (bsw_comcfg_cvc) is intentionally
        # NOT in this list — its generated Com_Cfg_Cvc.c is pure data (no
        # executable lines), so there is nothing to report.
    ]
...
def _bsw_comcfg_phase_to_line(p: BswComCfgPhase) -> str:
    """Serialize one BSW Com config readback phase to key=value script lines."""
    tokens = [f"op={p.op}"]
    if p.op in ("txpdu", "rxpdu", "sig"):
        tokens.append(f"id={p.id}")
    if p.op == "sig":
        tokens.append(f"table={p.table}")
    return " ".join(tokens)


@app.post("/api/test/bsw/comcfg/cvc/setup")
def setup_bsw_comcfg_cvc(body: BswComCfgSetupBody):
    """Store the CVC Com config readback phase script for subsequent run calls."""
    global _stored_bsw_comcfg_cvc_phases
    _stored_bsw_comcfg_cvc_phases = body.phases
    return {"phaseCount": len(body.phases)}


@app.post("/api/test/bsw/comcfg/cvc")
def run_bsw_comcfg_cvc(body: BswComCfgRunBody):
    """Read back the generated CVC Com configuration in a native test harness.

    The `/setup` endpoint stores the precondition phase script (Given context).
    `body.phases` carries the stimulus phases. The harness links the generated
    `Com_Cfg_Cvc.c` (GENERATED BY arxmlgen -- DO NOT EDIT) and walks the
    exported `const Com_ConfigType cvc_com_config` data tables, verifying the
    TX/RX PDU tables, signal bit definitions and structural invariants match
    the DBC-derived expectations. No BSW module logic is linked.
    """
    stimulus = body.phases if body.phases is not None else []
    phases = list(_stored_bsw_comcfg_cvc_phases) + list(stimulus)
    if not phases:
        raise HTTPException(status_code=400, detail="No phases provided (run /setup first)")

    script = "\n".join(_bsw_comcfg_phase_to_line(p) for p in phases) + "\n"
    try:
        env = os.environ.copy()
        env["LLVM_PROFILE_FILE"] = os.path.join(_COVERAGE_DIR, "bsw_comcfg_cvc_%p.profraw")
        completed = subprocess.run(
            [_BSW_COMCFG_CVC_HARNESS],
            input=script,
            capture_output=True,
            text=True,
            timeout=30,
        ...
        raise HTTPException(status_code=500, detail="BSW Com config harness not found") from exc
    except subprocess.TimeoutExpired as exc:
        raise HTTPException(status_code=504, detail="BSW Com config harness timed out") from exc
    if completed.returncode != 0:
        detail = completed.stderr.strip() if completed.stderr else completed.stdout.strip()
        raise HTTPException(status_code=500, detail=detail or "BSW Com config harness failed")

    try:
        return json.loads(completed.stdout)
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=500, detail="BSW Com config harness returned invalid JSON") from exc


def _bsw_rtetaskbodies_phase_to_line(p: BswRteTaskBodiesPhase) -> str:
    """Serialize one BSW RTE task-bodies phase to key=value script lines."""
    tokens = [f"op={p.op}", f"task={p.task}"]
    if p.task == "idle":
        tokens.append(f"idleIters={p.idleIters}")
    return " ".join(tokens)


@app.post("/api/test/bsw/rtetaskbodies/cvc/setup")
def setup_bsw_rtetaskbodies_cvc(body: BswRteTaskBodiesSetupBody):
    """Store the CVC RTE task-bodies phase script for subsequent run calls."""
    global _stored_bsw_rtetaskbodies_cvc_phases
    _stored_bsw_rtetaskbodies_cvc_phases = body.phases
    return {"phaseCount": len(body.phases)}


@app.post("/api/test/bsw/rtetaskbodies/cvc")
def run_bsw_rtetaskbodies_cvc(body: BswRteTaskBodiesRunBody):
    """Drive the generated CVC RTE task bodies in a native test harness.

    The `/setup` endpoint stores the precondition phase script (Given context).
    `body.phases` carries the stimulus phases. The harness links the generated
    `Rte_TaskBodies_Cvc.c` (GENERATED BY arxmlgen -- DO NOT EDIT) and runs each
    OSEK task body against mocked OS/runnable dependencies, verifying the exact
    S-OS-11 runnable dispatch order and WdgM checkpoint placement.
    """
    stimulus = body.phases if body.phases is not None else []
    phases = list(_stored_bsw_rtetaskbodies_cvc_phases) + list(stimulus)
    if not phases:
        raise HTTPException(status_code=400, detail="No phases provided (run /setup first)")

    script = "\n".join(_bsw_rtetaskbodies_phase_to_line(p) for p in phases) + "\n"
    try:
        env = os.environ.copy()
        env["LLVM_PROFILE_FILE"] = os.path.join(_COVERAGE_DIR, "bsw_rtetaskbodies_cvc_%p.profraw")
        completed = subprocess.run(
            [_BSW_RTETASKBODIES_CVC_HARNESS],
            input=script,
            capture_output=True,
            text=True,
            timeout=30,
        ...
        raise HTTPException(status_code=500, detail="BSW RTE task-bodies harness not found") from exc
    except subprocess.TimeoutExpired as exc:
        raise HTTPException(status_code=504, detail="BSW RTE task-bodies harness timed out") from exc
    if completed.returncode != 0:
        detail = completed.stderr.strip() if completed.stderr else completed.stdout.strip()
        raise HTTPException(status_code=500, detail=detail or "BSW RTE task-bodies harness failed")

    try:
        return json.loads(completed.stdout)
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=500, detail="BSW RTE task-bodies harness returned invalid JSON") from exc
