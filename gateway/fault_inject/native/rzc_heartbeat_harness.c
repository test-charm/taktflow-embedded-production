/**
 * @file    rzc_heartbeat_harness.c
 * @brief   Native test harness for Swc_Heartbeat (RZC heartbeat SWC)
 * @date    2026-08-17
 *
 ...
 *            vehicleState uint32  RTE RZC_SIG_VEHICLE_STATE input read at TX
 *                                  boundary (0=INIT 1=RUN 2=DEGRADED 3=LIMP
 *                                  4=SAFE_STOP 5=SHUTDOWN)
 *            faultMask   uint32   RTE RZC_SIG_FAULT_MASK input read at TX
 *                                  boundary (bit 3 = RZC_FAULT_CAN; TX is
 *                                  suppressed only when the CAN fault bit is
 *                                  set AND vehicle_state == SAFE_STOP)
 *
 *          Output is a single JSON object on stdout:
 *            {"aliveCounter":0,"cycleCounter":0,"initialized":1,
 *             "ecuId":3,"operatingMode":0,"faultStatus":0,"alive":0}
 *
...
int main(void)
{
    ...
    printf("{\"aliveCounter\":%u,\"cycleCounter\":%u,\"initialized\":%u,"
           ...
           (unsigned)mock_rte_signals[RZC_SIG_RZC_HEARTBEAT_ECU_ID],
           (unsigned)mock_rte_signals[RZC_SIG_RZC_HEARTBEAT_OPERATING_MODE],
           (unsigned)mock_rte_signals[RZC_SIG_RZC_HEARTBEAT_FAULT_STATUS],
           (unsigned)mock_rte_signals[RZC_SIG_HEARTBEAT_ALIVE]);
